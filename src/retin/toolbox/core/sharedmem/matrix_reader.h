...
template<class T> void general_fvec_reader<T>::read (T*& buffer,size_t truncateDim) {
    if (!buffer) buffer = new T[ get_width()*get_height() ];
...
template<class T> void general_bin_reader<T>::read (T*& data,size_t truncateDim) {
	T x = 0;
