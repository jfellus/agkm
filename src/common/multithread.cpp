...
typedef struct {void(*f)(int,int,int); int first,nb,th;} _pthread_do_multithread_range_data;

void* _pthread_do_multithread_range(void* d) {
	_pthread_do_multithread_range_data* data = (_pthread_do_multithread_range_data*)d;

	data->f(data->th, data->first,data->nb);
...
void do_multithread(void (*f)(int th, int start,int end), int n) {
	...
	for(int i=0; i<nbthreads; i++) {
		...
		d->f = f; d->first = first; d->nb = nb; d->th = i;
